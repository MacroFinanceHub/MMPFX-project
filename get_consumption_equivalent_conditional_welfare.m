...
global oo_ M_ options_ Wbase

if par_value_lambda>1 %do not allow negative consumption
    outvalue=1e5+par_value_lambda^2;
end

set_param_value('lambda_cost',par_value_lambda)  %set consumption equivalent lambda
...
if ~options_.ramsey_policy
    outvalue= y_sim(strmatch('U',M_.endo_names,'exact'),2)-Wbase;%y_sim(strmatch('Welfare_gap',M_.endo_names,'exact'),2)*100; %read out gap
else
